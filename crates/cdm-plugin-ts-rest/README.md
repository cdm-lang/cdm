...
The `@tsRest` plugin must be listed **after** the `@typescript` plugin in the CDM file to ensure its outputs are available.

**Note:** Enable `generate_zod: true` in the `@typescript` plugin configuration to generate Zod schemas that the ts-rest contract will import.
...
```typescript
/**
 * Generated by CDM @tsRest plugin
...
| `path` | `string` | `"./schemas"` | For `single`: file path without extension. For `per_model`: directory path |
...
The `@tsRest` plugin needs to know where to import Zod schemas from. This depends on how you configure the `@typescript` plugin.
...
import { GetUserPathParamsSchema } from './models/GetUserPathParams';
import { NotFoundErrorSchema } from './models/NotFoundError';
...
This file imports Zod schemas from the location configured via `schema_import` (default: `./schemas`). The schemas are generated by the `@typescript` plugin with `generate_zod: true`.
...
```cdm
@tsRest from "./path/to/cdm-plugin-ts-rest" {
  build_output: "./generated",
  routes: { /* ... */ }
}
```
