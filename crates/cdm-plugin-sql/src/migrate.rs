...
/// Collects SQL statements into ordered categories for correct emission.
/// Ensures operations like DROP INDEX happen before DROP COLUMN, and
/// ADD COLUMN happens before CREATE INDEX.
struct MigrationStatements {
    ...
    rename_tables: Vec<String>,
    create_indexes: Vec<String>,
    pk_comments: Vec<String>,
}
...
impl MigrationStatements {
    ...
    fn emit(&self) -> String {
        ...
        for s in &self.comments { result.push_str(s); }
        for s in &self.drop_indexes { result.push_str(s); }
        for s in &self.drop_tables { result.push_str(s); }
        for s in &self.alter_columns { result.push_str(s); }
        for s in &self.add_columns { result.push_str(s); }
        for s in &self.create_tables { result.push_str(s); }
        for s in &self.rename_tables { result.push_str(s); }
        for s in &self.create_indexes { result.push_str(s); }
        for s in &self.pk_comments { result.push_str(s); }
        result
    }
}
...
pub fn migrate(
    ...
    // Generate up and down migration contents using ordered buckets
    let mut up_stmts = MigrationStatements::new();
    let mut down_stmts = MigrationStatements::new();
    ...
    for delta in &sorted_deltas {
        match delta {
            Delta::ModelAdded { name, after } => {
                ...
                up_stmts.create_tables.push(format!("{}\n", table_sql));
                ...
                down_stmts.drop_tables.push(format!("DROP TABLE {};\n\n", full_table_name));
            }
            ...
            Delta::ModelRemoved { name, before } => {
                ...
                up_stmts.drop_tables.push(format!("DROP TABLE {};\n\n", full_table_name));

                // DOWN: CREATE TABLE
                let table_sql = generate_create_table(name, before, &config, &type_mapper);
                down_stmts.create_tables.push(format!("{}\n", table_sql));
            }
            ...
            Delta::ModelRenamed { old_name, new_name, id: _, before, after } => {
                ...
                match dialect {
                    Dialect::PostgreSQL => {
                        ...
                        up_stmts.rename_tables.push(format!(
                        ...
                        down_stmts.rename_tables.push(format!(
                            "ALTER TABLE {} RENAME TO {};\n\n",
                            full_new_name,
                            quote_identifier(&old_table_name, dialect)
                        ));
            ...
            Delta::FieldAdded { model, field, after } => {
                ...
                up_stmts.add_columns.push(format!(
            ...
            Delta::FieldRemoved { model, field, before } => {
                ...
                up_stmts.alter_columns.push(format!(
                    "ALTER TABLE {} DROP COLUMN {};\n\n",
                    full_table_name,
                    quote_identifier(&column_name, dialect)
                ));
                ...
                let column_def = generate_column_definition(field_def, &column_name, &config, &type_mapper);
                down_stmts.add_columns.push(format!(
                    "ALTER TABLE {} ADD COLUMN {};\n\n",
                    full_table_name,
                    column_def
                ));
            ...
            Delta::TypeAliasAdded { name, after: _ } => {
                up_stmts.comments.push(format!("-- Type alias added: {}\n\n", name));
                down_stmts.comments.push(format!("-- Type alias removed: {}\n\n", name));
            }

            Delta::TypeAliasRemoved { name, before: _ } => {
                up_stmts.comments.push(format!("-- Type alias removed: {}\n\n", name));
                down_stmts.comments.push(format!("-- Type alias added: {}\n\n", name));
            }

            Delta::TypeAliasRenamed { old_name, new_name, id: _, before: _, after: _ } => {
                up_stmts.comments.push(format!("-- Type alias renamed: {} -> {}\n\n", old_name, new_name));
                down_stmts.comments.push(format!("-- Type alias renamed: {} -> {}\n\n", new_name, old_name));
            }

            Delta::TypeAliasTypeChanged { name, before: _, after: _ } => {
                up_stmts.comments.push(format!("-- Type alias changed: {}\n\n", name));
                down_stmts.comments.push(format!("-- Type alias changed: {}\n\n", name));
            }

            Delta::InheritanceAdded { model, parent } => {
                up_stmts.comments.push(format!("-- Inheritance added: {} extends {}\n\n", model, parent));
                down_stmts.comments.push(format!("-- Inheritance removed: {} extends {}\n\n", model, parent));
            }

            Delta::InheritanceRemoved { model, parent } => {
                up_stmts.comments.push(format!("-- Inheritance removed: {} extends {}\n\n", model, parent));
                down_stmts.comments.push(format!("-- Inheritance added: {} extends {}\n\n", model, parent));
            }

            Delta::GlobalConfigChanged { before: _, after: _ } => {
                up_stmts.comments.push("-- Global config changed\n\n".to_string());
                down_stmts.comments.push("-- Global config changed\n\n".to_string());
            }
                        ...
                        if !exists_in_before {
                            if after_idx.is_primary {
                                ...
                                if is_actual_pk_change {
                                    up_stmts.pk_comments.push(format!(
                                        "-- Primary key change requires manual migration: {}\n\n",
                                        after_idx.name
                                    ));
                                    down_stmts.pk_comments.push(format!(
                                        "-- Primary key change requires manual migration: {}\n\n",
                                        after_idx.name
                                    ));
                                }
                                ...
                                let mut sql = generate_create_index_sql(after_idx, &table_name, &schema_prefix_str, dialect);
                                sql.push('\n');
                                up_stmts.create_indexes.push(sql);

                                // Generate DROP INDEX for down migration
                                let mut sql = generate_drop_index_sql(after_idx, &table_name, &schema_prefix_str, dialect);
                                sql.push('\n');
                                down_stmts.drop_indexes.push(sql);
                            }
                        }
                    }
                    ...
                    for before_idx in &before_indexes {
                        ...
                        if !exists_in_after {
                            if before_idx.is_primary {
                                ...
                                let mut sql = generate_drop_index_sql(before_idx, &table_name, &schema_prefix_str, dialect);
                                sql.push('\n');
                                up_stmts.drop_indexes.push(sql);

                                // Generate CREATE INDEX for down migration
                                let mut sql = generate_create_index_sql(before_idx, &table_name, &schema_prefix_str, dialect);
                                sql.push('\n');
                                down_stmts.create_indexes.push(sql);
                            }
                        }
                    }
                } else {
                    up_stmts.comments.push(format!("-- Model config changed: {}\n\n", model));
                    down_stmts.comments.push(format!("-- Model config changed: {}\n\n", model));
                }
            }
            ...
            Delta::FieldConfigChanged { model, field, before, after } => {
                ...
                if let Some(model_def) = current_schema.models.get(model) {
                    ...
                    if !has_sql_changes {
                        up_stmts.comments.push(format!("-- Field config changed: {}.{}\n\n", model, field));
                        down_stmts.comments.push(format!("-- Field config changed: {}.{}\n\n", model, field));
                    }
                } else {
                    up_stmts.comments.push(format!("-- Field config changed: {}.{}\n\n", model, field));
                    down_stmts.comments.push(format!("-- Field config changed: {}.{}\n\n", model, field));
    ...
    up_content.push_str(&up_stmts.emit());
    ...
    down_content.push_str(&down_stmts.emit());
