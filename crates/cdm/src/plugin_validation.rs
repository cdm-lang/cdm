...
impl PluginCache {
    ...
    fn resolve_plugin_path(&self, import: &PluginImport) -> Result<PathBuf, String> {
        match &import.source {
            ...
            None => {
                // Try local path first, then fall back to registry
...
fn resolve_from_registry(plugin_name: &str, config: &Option<JSON>) -> Result<PathBuf, String> {
...
fn resolve_git_plugin(url: &str, plugin_name: &str, config: &Option<JSON>) -> Result<PathBuf, String> {
