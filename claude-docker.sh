#!/bin/bash
# Helper script to run Claude Code CLI in Docker
...
if [ -n "$CUSTOM_NAME" ]; then
    ...
    BRANCH_NAME="claude-${SANITIZED_NAME}"
    WORKTREE_NAME="claude-${SANITIZED_NAME}"
else
    SESSION_ID="${TIMESTAMP}"
    BRANCH_NAME="claude-${TIMESTAMP}"
    WORKTREE_NAME="claude-${TIMESTAMP}"
fi

export CONTAINER_NAME="cdm-claude-${SESSION_ID}"
export BRANCH_NAME
export WORKTREE_NAME

echo -e "${BLUE}Starting Claude Code CLI in Docker...${NC}"
...
cat > ".claude/sessions/${SESSION_ID}" <<EOF
...
if [ "$BUILD_FLAG" = true ] || ! docker images | grep -q "cdm-claude"; then
...
echo -e "${BLUE}To open a shell in this session, run: ./claude-shell.sh ${SESSION_ID}${NC}"
docker-compose -f docker-compose.claude.yml run --rm --name "${CONTAINER_NAME}" claude

# Cleanup session file when done
rm -f ".claude/sessions/${SESSION_ID}"
